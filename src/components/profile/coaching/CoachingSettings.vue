<template>
  ...
  <div class="w-full grid grid-cols-4 gap-4">
    ...
    <div
      ...
      <div v-for="coaching in coachings" v-show="coaching.active" :key="`coaching${coaching.coachingId}`"
           class="flex flex-col items-center p-2 gap-1" :data-coachingId="coaching.coachingId"
           @click="console.log(coaching.coachingId)">
        <p class="bg-mainBlue text-white px-4 py-2 rounded-3xl">{{ coaching.subject }}</p>
        <small class="text-white font-thin">{{ coaching.level }}</small>
        <small class="text-white">{{ coaching.rate }}€ / Stunde</small>
        <small v-if="idToRemove !== coaching.coachingId"
               class="text-mainYellow underline hover:text-red-700 hover:cursor-pointer"
               @click="() => {idToRemove = coaching.coachingId}">Löschen</small>
        <div v-else>
          <small @click="() => {idToRemove = -1}" class="mr-1 hover:cursor-pointer">zurück</small>
          <small @click="() => {deleteCoaching(coaching.coachingId, coaching.level, coaching.rate)}"
                 class="text-red-700 underline hover:cursor-pointer">löschen</small>
        </div>
      </div>
    </div>
